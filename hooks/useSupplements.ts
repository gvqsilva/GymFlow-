...
export function useSupplements() {
  ...
  const addSupplement = async (supplement: Omit<Supplement, "id">) => {
    ...
    ToastPresets.success(
      "Suplemento adicionado!",
      "Suplemento adicionado com sucesso!",
    );
  };
  ...
  const deleteSupplement = async (supplementId: string) => {
    ...
    const supplementName =
      currentSupplements.find((s) => s.id === supplementId)?.name ||
      "Suplemento";
    ...
    ToastPresets.info(
      "Suplemento removido",
      `${supplementName} foi excluído da lista.`,
    );
  };
