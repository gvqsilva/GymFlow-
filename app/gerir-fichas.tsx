...
export default function ManageWorkoutsScreen() {
  ...
  type WorkoutListItem = Workout & { storageKey: string };
  ...
  const workoutsList: WorkoutListItem[] = Object.entries(workouts).map(
    ([storageKey, workout]) => ({
      ...workout,
      id: workout?.id || storageKey,
      storageKey,
    }),
  );
  ...
  return (
    <View style={styles.container}>
      ...
      <FlatList
        ...
        renderItem={({ item }: { item: WorkoutListItem }) => (
          <Pressable
            style={styles.card}
            onPress={() =>
              router.push({
                pathname: "/editar-ficha/[id]",
                params: { id: item.storageKey, title: item.name },
              })
            }
          >
            ...
            <Pressable
              onPress={(event) => {
                ...
                handleDeleteWorkout(item.storageKey, item.name);
              }}
      ...
      <Pressable
        ...
        onPress={() => router.push("/ficha-modal")}
      >
