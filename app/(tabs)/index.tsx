...
import {
  normalizeSupplementsHistoryByDate,
  upsertSupplementValueByDate,
} from "../../utils/supplementsHistoryUtils";
...
const HOME_QUICK_ACTION_OPTIONS: Array<{
...
}> = [
  {
    ...
    route: "/planejar-semana",
  },
  {
    id: "gerir-suplementos",
    label: "Suplementos",
    icon: "medical-outline",
    route: "/gerir-suplementos",
  },
  {
    id: "musculacao",
    label: "Treinos",
  ...
  return (
    <Animated.View
      style={[
        styles.metricCard,
        {
          transform: [{ scale: scaleAnim }, { scale: bounceAnim }, { rotate }],
          opacity: fadeAnim,
...
const QuickStat = ({
  ...
  return (
    ...
    >
      <Animated.View
        style={[styles.quickStatContent, { transform: [{ scale: scaleAnim }] }]}
      >
    ...
    return (
      <View style={styles.graphContainer}>
        ...
        <Text style={styles.noActivityText}>
          Nenhuma atividade registada esta semana.{" "}
        </Text>
      </View>
    );
  }
...
export default function HomeScreen() {
  ...
  const { reloadData: reloadWeeklyPlan } = useWeeklyPlan();
  ...
  const [userName, setUserName] = useState(
    currentUserConfig?.name || "Utilizador",
  );
  ...
  const [isDailySummaryModalVisible, setIsDailySummaryModalVisible] =
    useState(false);
  ...
  const selectedQuickActionIds = useMemo<HomeQuickActionId[]>(
    () =>
      Array.isArray(currentUserConfig?.quickActions) &&
      currentUserConfig.quickActions.length > 0
        ? currentUserConfig.quickActions.slice(0, MAX_QUICK_ACTIONS)
        : DEFAULT_HOME_QUICK_ACTIONS,
    [currentUserConfig],
  );
  ...
  const selectedQuickActions = useMemo(
    () =>
      HOME_QUICK_ACTION_OPTIONS.filter((option) =>
        selectedQuickActionIds.includes(option.id),
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      try {
        ...
        const firebaseWorkoutHistory =
          await firebaseSyncService.loadWorkoutHistory();
        if (firebaseWorkoutHistory && firebaseWorkoutHistory.length > 0) {
          await AsyncStorage.setItem(
            "workoutHistory",
            JSON.stringify(firebaseWorkoutHistory),
          );
          console.log("✅ Histórico de treinos carregado do Firebase");
        }

        // Carrega histórico de suplementos
        const firebaseSupplementsHistory =
          await firebaseSyncService.loadSupplementsHistory();
        if (firebaseSupplementsHistory) {
          await AsyncStorage.setItem(
            SUPPLEMENTS_HISTORY_KEY,
            JSON.stringify(firebaseSupplementsHistory),
          );
      ...
      if (workoutHistoryJSON) {
        const history: {
          date: string;
          category: string;
          details: { calories?: number; type?: string; duration?: number };
        }[] = JSON.parse(workoutHistoryJSON);

        const startOfWeekString = getLocalDateString(getStartOfWeek());
        const weeklyHistory = history.filter(
          (entry) => entry.date >= startOfWeekString,
        );

        const gymWorkoutsThisWeek = weeklyHistory.filter(
          (entry) => entry.category === "Musculação",
        );
        setWeeklyGymWorkouts(gymWorkoutsThisWeek.length);

        const summary = weeklyHistory.reduce(
          (acc, entry) => {
        ...
        const totalKcal = activitiesToday.reduce(
          (sum, entry) => sum + (entry.details?.calories || 0),
          0,
        );
  ...
  const checkAchievements = useCallback(
    (newCalories: number, newActivities: number, newWorkouts: number) => {
      const achievements = [];

      // Conquista de calorias
      if (
        newCalories >= userGoals.dailyCalories &&
        totalCaloriesToday < userGoals.dailyCalories
      ) {
        achievements.push({
      ...
      if (
        newActivities >= userGoals.weeklyActivities &&
        Object.values(weeklySummary).reduce((sum, count) => sum + count, 0) <
          userGoals.weeklyActivities
      ) {
        achievements.push({
          title: "💪 Semana Ativa!",
      ...
      if (
        newWorkouts >= userGoals.weeklyWorkouts &&
  ...
  useEffect(() => {
    if (isFocused) {
      ...
      refreshWorkouts();
      refreshSupplements();
      reloadWeeklyPlan(); // Recarrega plano semanal quando a tela volta ao foco
  ...
  const updateSupplementValue = async (
    ...
    try {
      if (typeof newValue === "boolean" && !newValue) {
        ...
        await supplementsHistory.markSupplementNotTaken(
          supplement.id,
          supplement.name,
          today,
        );
        ...
        const previousValue =
          supplementsHistory.getSupplementStatus(supplement.id, today)
            ?.timesTaken || 0;
        if (newValue < previousValue) {
          // Remover dose
          await supplementsHistory.updateSupplementEntry(supplement.id, today, {
            timesTaken: newValue,
          });
        } else if (newValue > previousValue) {
          // Adicionar dose
          await supplementsHistory.markSupplementTaken(
            supplement.id,
            supplement.name,
            today,
            supplement.dose,
      ...
      const legacySupplementsHistoryJSON = await AsyncStorage.getItem(
        SUPPLEMENTS_HISTORY_KEY,
      );
      const legacyRawHistory = legacySupplementsHistoryJSON
        ? JSON.parse(legacySupplementsHistoryJSON)
        : {};
      const legacyNormalized =
        normalizeSupplementsHistoryByDate(legacyRawHistory);
      const legacyUpdated = upsertSupplementValueByDate(
        legacyNormalized,
        today,
        supplement.id,
        typeof newValue === "number" ? Math.max(0, newValue) : !!newValue,
      );
      await AsyncStorage.setItem(
        SUPPLEMENTS_HISTORY_KEY,
        JSON.stringify(legacyUpdated),
      );

      // Firebase sync é automático através do hook
      console.log(
        "✅ Histórico de suplementos atualizado automaticamente no Firebase",
      );
  ...
  const today = getLocalDateString();
  const visibleSupplements = supplements.filter(
    (s: any) => s.showOnHome !== false,
  );
  const supplementsDoneCount = visibleSupplements.reduce(
    (count: number, supplement: any) => {
      const status = supplementsHistory.getSupplementStatus(
        supplement.id,
        today,
      );
      if (supplement.trackingType === "counter") {
        return count + (Number(status?.timesTaken || 0) > 0 ? 1 : 0);
      }
      return count + (status?.taken ? 1 : 0);
    },
    0,
  );

  const totalDurationToday = todayActivities.reduce(
    (sum, entry) =>
      sum +
      (entry.details?.duration || (entry.category === "Musculação" ? 60 : 0)),
    0,
  );

  const todaySupplementEntries =
    supplementsHistory.getSupplementHistoryForDate(today);

  const todayFoodEntries = (foodHistory.foodHistory || []).filter(
    (entry: any) => entry.date === today,
  );

  const totalFoodCaloriesToday = todayFoodEntries.reduce(
    (sum: number, entry: any) => sum + Number(entry.calories || 0),
    0,
  );

  const hasAnyRecordToday =
    todayActivities.length > 0 ||
    todaySupplementEntries.length > 0 ||
    todayFoodEntries.length > 0;

  const supplementEntriesToday = visibleSupplements.reduce(
    (total: number, supplement: any) => {
      ...
      if (supplement.trackingType === "counter") {
        return total + Number(status?.timesTaken || 0);
      }
      return total + (status?.taken ? 1 : 0);
    },
    0,
  );

  const totalWeeklyActivities = Object.values(weeklySummary).reduce(
    (sum, count) => sum + count,
    0,
  );
  ...
  return (
    <SafeAreaView style={styles.safeArea}>
      ...
      <ScrollView showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.metricsContainer}>
          ...
          {visibleMetrics.activities && (
            <AnimatedMetric
              ...
              onPress={() => {
                setExpandedMetric(
                  expandedMetric === "activities" ? null : "activities",
                );
              }}
          ...
          <View style={styles.weeklyProgressHeader}>
            ...
            <View style={styles.weeklyTitleContainer}>
              ...
              <Text style={styles.weeklySubtitle}>
                {weeklyGymWorkouts}/{userGoals.weeklyWorkouts} treinos{" "}
              </Text>
            </View>
          ...
          <View style={styles.weeklyProgressContent}>
            ...
            {weeklyGymWorkouts >= userGoals.weeklyWorkouts ? (
              <View style={styles.weeklyCompletedBadge}>
                ...
                <Text style={styles.weeklyCompletedText}>
                  Meta Concluída! 🎉{" "}
                </Text>
              </View>
            ) : (
        ...
        {/* ✅ NOVO: Card com plano de atividades para hoje */}
        <TodaysPlanCard />
        ...
        <View style={styles.dailyOverviewCard}>
          <View style={styles.dailyOverviewHeader}>
            <View>
              <Text style={styles.dailyOverviewTitle}>Resumo do Dia</Text>
              <Text style={styles.dailyOverviewSubtitle}>
                Somente registros feitos hoje
              </Text>
            </View>
          </View>

          <View style={styles.dailyOverviewStatsGrid}>
            <View style={styles.dailyOverviewStatCard}>
              <Text style={styles.dailyOverviewLabel}>Atividades</Text>
              <Text style={styles.dailyOverviewStatValue}>
                {todayActivities.length}
              </Text>
            </View>

            <View style={styles.dailyOverviewStatCard}>
              <Text style={styles.dailyOverviewLabel}>Calorias</Text>
              <Text style={styles.dailyOverviewStatValue}>
            ...
            <View style={styles.dailyOverviewStatCard}>
              <Text style={styles.dailyOverviewLabel}>Tempo</Text>
              <Text style={styles.dailyOverviewStatValue}>
                {Math.round(totalDurationToday)} min
              </Text>
            </View>

            <View style={styles.dailyOverviewStatCard}>
              <Text style={styles.dailyOverviewLabel}>Suplementos</Text>
              <Text style={styles.dailyOverviewStatValue}>
                {supplementEntriesToday}
              </Text>
            </View>
          </View>

          <Pressable
            style={styles.dailyOverviewAction}
            onPress={() => setIsDailySummaryModalVisible(true)}
          >
            <Ionicons
              ...
              color={theme.colors.primary}
            />
            <Text style={styles.dailyOverviewActionText}>
              Ver detalhes do dia
            </Text>
          </Pressable>
        </View>
        ...
        <View style={styles.content}>
          {/* ✅ ATUALIZADO: Card de atalho para fichas de treino */}
          <View style={[styles.nextWorkoutCard, styles.featuredCard]}>
            <View style={styles.nextWorkoutHeader}>
              ...
              <View style={styles.nextWorkoutInfo}>
                <Text style={styles.nextWorkoutTitle}>Academia</Text>
                <Text style={styles.nextWorkoutName}>
                  Acesse suas fichas de treino
                </Text>
              </View>
            </View>
            <Link href="/musculacao" asChild>
              <TouchableOpacity style={styles.startWorkoutButton}>
                <Text style={styles.startWorkoutText}>Ir para Academia</Text>
          ...
          {visibleSupplements.length > 0 && (
            <View style={styles.supplementsSection}>
              ...
              {visibleSupplements.map((supplement: any, index: number) => {
                const supplementStatus = supplementsHistory.getSupplementStatus(
                  supplement.id,
                  today,
                );
                  ...
                  return (
                    ...
                    >
                      ...
                      >
                        ...
                        <View
                          style={[
                            styles.supplementStatusIcon,
                            isTaken && styles.supplementStatusIconTaken,
                          ]}
                        >
                          <Ionicons
                            name={
                ...
                if (supplement.trackingType === "counter") {
                  ...
                  return (
                    <View
                      key={supplement.id}
                      style={styles.supplementCardEnhanced}
                    >
                      <View style={styles.supplementContent}>
                        ...
                        <View style={styles.counterEnhanced}>
                          <TouchableOpacity
                            ...
                            style={[
                              styles.counterButtonEnhanced,
                              styles.counterButtonMinus,
                            ]}
                          >
                            <Ionicons name="remove" size={18} color="white" />
                          </TouchableOpacity>
                          <Text style={styles.counterTextEnhanced}>
                            {count}{" "}
                          </Text>
                          <TouchableOpacity
                            onPress={() => {
            ...
            <FlatList
              ...
              renderItem={({ item }) => {
                ...
                if (item.category === "Musculação" && item.details?.type) {
                  const workoutName =
                    workouts && workouts[item.details.type]
                      ? workouts[item.details.type].name
                      : item.details.type;
                ...
                const normalizedCategory = (item.category || "")
                  ...
                  .normalize("NFD")
                  .replace(/[\u0300-\u036f]/g, "");
                const isSwimming = normalizedCategory === "natacao";
                ...
                if (item.details?.duration) {
                  detailParts.push(`${item.details.duration} min`);
                }
                if (isSwimming && item.details?.distance) {
                  detailParts.unshift(`${item.details.distance} m`);
                }
                if (isSwimming && item.details?.pace) {
                  detailParts.push(String(item.details.pace));
                }
                if (isRunningOrCycling && item.details?.distanceKm) {
                  detailParts.unshift(`${item.details.distanceKm} km`);
                }
                if (isRunningOrCycling && item.details?.pace) {
                  detailParts.push(String(item.details.pace));
                }
                const activityDetails = detailParts.join(" • ");
                ...
                return (
                  <View style={styles.activityItem}>
                    <View style={styles.activityInfo}>
                      ...
                      {activityDetails ? (
                        <Text style={styles.activityMeta}>{activityDetails}</Text>
                      ) : null}
                    </View>
                    <Text style={styles.activityCalories}>
                      {item.details?.calories || 0} kcal
                    </Text>
                  </View>
                );
              }}
              ListEmptyComponent={
                <Text style={styles.noActivityTextModal}>
      ...
      <Modal
        animationType="slide"
        transparent={true}
        visible={isDailySummaryModalVisible}
        onRequestClose={() => setIsDailySummaryModalVisible(false)}
      >
        <Pressable
          style={styles.modalContainer}
          onPress={() => setIsDailySummaryModalVisible(false)}
        >
          <Pressable style={styles.modalContent}>
            ...
            >
              ...
              <View style={styles.detailsSection}>
                <Text style={styles.detailsSectionTitle}>
                  Atividades ({todayActivities.length})
                </Text>
                {todayActivities.length === 0 ? (
                  <Text style={styles.noActivityTextModal}>
                  ...
                  todayActivities.map((item, index) => {
                    ...
                    return (
                      <View
                        key={`summary-${item.category}-${index}`}
              ...
              <View style={styles.detailsSection}>
                <Text style={styles.detailsSectionTitle}>
                  Suplementos ({todaySupplementEntries.length})
                </Text>
                {todaySupplementEntries.length === 0 ? (
                  <Text style={styles.noActivityTextModal}>
                  ...
                  todaySupplementEntries.map((entry: any, index: number) => (
                    <View
                      key={`${entry.supplementId}-${index}`}
                      style={styles.activityItem}
                    >
                      <View style={styles.activityInfo}>
                        <Text style={styles.activityName}>
                        ...
                        <Text style={styles.activityMeta}>
                          {entry.taken
                            ? `Tomado${entry.timesTaken && entry.timesTaken > 1 ? ` (${entry.timesTaken}x)` : ""}`
                            : "Não tomado"}
                          {entry.doseTaken
                            ? ` • Dose: ${entry.doseTaken}`
                            : ""}
                        </Text>
                      </View>
                    </View>
              ...
              <View style={styles.detailsSection}>
                <Text style={styles.detailsSectionTitle}>
                  Alimentação ({todayFoodEntries.length})
                </Text>
                {todayFoodEntries.length === 0 ? (
                  <Text style={styles.noActivityTextModal}>
                  ...
                  <>
                    {todayFoodEntries.map((entry: any) => (
                      <View key={entry.id} style={styles.activityItem}>
                        <View style={styles.activityInfo}>
                          <Text style={styles.activityName}>{entry.foodName}</Text>
                          <Text style={styles.activityMeta}>
                            {entry.quantity} {entry.unit} • P:{" "}
                            {entry.protein || 0}g • C: {entry.carbs || 0}g • G:{" "}
                            {entry.fat || 0}g
                          </Text>
                        </View>
                        <Text style={styles.activityCalories}>
                    ...
                    <View style={styles.detailsSectionTotalRow}>
                      <Text style={styles.detailsSectionTotalLabel}>
                        Total alimentação
                      </Text>
                      <Text style={styles.detailsSectionTotalValue}>
                        {totalFoodCaloriesToday} kcal
                      </Text>
                    </View>
                  </>
                )}
...
const styles = StyleSheet.create({
  ...
  detailsSectionTotalLabel: {
    fontSize: 13,
    color: "#666",
    fontWeight: "600",
  },
  detailsSectionTotalValue: {
    fontSize: 14,
    color: themeColor,
    fontWeight: "700",
  },
