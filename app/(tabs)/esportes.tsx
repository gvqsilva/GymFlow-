...
import { Href, Link, Stack, useFocusEffect } from "expo-router";
...
const MonthlyActivitySummary = ({
  ...
  const monthlySportHistory = history.filter((entry) => {
    ...
    const adjustedEntryDate = new Date(
      entryDate.valueOf() + entryDate.getTimezoneOffset() * 60 * 1000,
    );
    return (
      entry.category === categoryToFilter &&
      adjustedEntryDate.getMonth() === currentMonth
    );
  });
