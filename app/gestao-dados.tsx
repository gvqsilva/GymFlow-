...
const getDailySummaryAndSpent = async (
  ...
  try {
    ...
    const foodEntries: any[] = foodEntriesJSON
      ? JSON.parse(foodEntriesJSON)
      : [];
    const { totalCalories } = processDailyFoodData(
      Array.isArray(foodEntries) ? foodEntries : [],
      today,
    );

    const workoutHistoryJSON = await AsyncStorage.getItem("workoutHistory");
    const workoutEntries: any[] = workoutHistoryJSON
...
const aggregateDailyNetBalance = (
  foodEntries: any[],
  workoutEntries: any[],
) => {
  const dailyMap: Record<
    string,
    { consumed: number; spent: number; net: number }
  > = {};
  const processEntries = (entries: any[], type: "consumed" | "spent") => {
    (Array.isArray(entries) ? entries : []).forEach((entry) => {
      ...
      if (!dailyMap[dateStr]) {
        dailyMap[dateStr] = { consumed: 0, spent: 0, net: 0 };
      }
...
const buildActivityKey = (entry: any, index: number) => {
  const idPart = entry?.id ? String(entry.id) : "no-id";
  const datePart = entry?.date ? String(entry.date) : "no-date";
  const categoryPart = entry?.category ? String(entry.category) : "no-category";
  const typePart = entry?.details?.type
    ? String(entry.details.type)
    : "no-type";
  return `${idPart}-${datePart}-${categoryPart}-${typePart}-${index}`;
};
...
export default function DataManagementScreen() {
  ...
  const {
    supplementsHistory,
    getSupplementStatus,
    forceSync: forceSupplementsHistorySync,
    ...
    const loadHistoryForCalendar = async () => {
      try {
        ...
        try {
          ...
          const firebaseFoodHistory =
            await firebaseSyncService.loadFoodHistory();
          if (firebaseFoodHistory && firebaseFoodHistory.length > 0) {
            await AsyncStorage.setItem(
              "foodHistory",
              JSON.stringify(firebaseFoodHistory),
            );
        ...
        const workoutHistoryJSON = await AsyncStorage.getItem("workoutHistory");
        setWorkoutHistory(
          workoutHistoryJSON ? JSON.parse(workoutHistoryJSON) : [],
        );
  ...
  useFocusEffect(
    useCallback(() => {
      refreshSupplements();
      forceSupplementsHistorySync();
      loadAllInitialData();
    }, [refreshSupplements, forceSupplementsHistorySync, loadAllInitialData]),
  );
  ...
  const calculateModalBalance = useCallback(
    (day: string) => {
  ...
  const balanceMarkedDates = useMemo(() => {
    ...
    for (const dateStr in netBalanceMap) {
      ...
      if (
        netBalanceMap[dateStr].consumed > 0 ||
        netBalanceMap[dateStr].spent > 0
      ) {
        marked[dateStr] = {
  ...
  const handleDeleteActivity = (activityToDelete: any) => {
    Alert.alert(
      "Apagar Atividade?",
      ...
      [
        { text: "Cancelar" },
        {
          ...
          onPress: async () => {
            ...
            const historyIndex = currentHistory.findIndex((entry) => {
              if (activityToDelete?.id) {
                return entry?.id === activityToDelete.id;
              }

              if (entry === activityToDelete) {
                return true;
              }

              return (
                entry?.date === activityToDelete?.date &&
                entry?.category === activityToDelete?.category &&
                JSON.stringify(entry?.details || {}) ===
                  JSON.stringify(activityToDelete?.details || {})
              );
            });
            ...
            if (historyIndex === -1) {
              Toast.show({
                ...
                text2: "Registo não encontrado no histórico.",
              });
              return;
            }
            ...
            try {
              ...
              console.log(
                "✅ Histórico de treinos sincronizado com Firebase após exclusão",
              );
            } catch (syncError) {
              console.warn(
                "⚠️ Falha na sincronização com Firebase:",
                syncError,
              );
            }
            ...
            Toast.show({
              type: "success",
              text1: "Atividade apagada com sucesso.",
            });
          },
        },
      ],
  ...
  const activityOptions = useMemo((): ActivityOption[] => {
    ...
    const gymWorkouts: ActivityOption[] = sortWorkoutsByName(
      workouts || {},
    ).map((w: any) => ({
      id: w.id,
      name: w.name,
    ...
    return [
      ...
      {
        id: "divider",
        name: "Fichas de Treino (Musculação)",
        isSport: "divider" as any,
      },
      ...gymWorkouts,
    ];
  }, [workouts, sports]);
  ...
  const handleAddActivitySelect = (item: ActivityOption) => {
    ...
    if (item.isSport === true) {
      router.push({
        pathname: "/logEsporte",
        params: { esporte: item.name, date: selectedDay },
      });
  ...
  const selectedDayActivities = (workoutHistory || []).filter(
    (entry) => entry.date === selectedDay,
  );
  const supplementsOnSelectedDay = useMemo(() => {
    if (!selectedDay) return {} as Record<string, boolean | number>;

    return supplements.reduce(
      (acc, supplement) => {
        const status = getSupplementStatus(supplement.id, selectedDay);
        acc[supplement.id] =
          supplement.trackingType === "counter"
            ? Number(status?.timesTaken || 0)
            : Boolean(status?.taken);
        return acc;
      },
      {} as Record<string, boolean | number>,
    );
  }, [selectedDay, supplements, supplementsHistory, getSupplementStatus]);
  const hasConfiguredSupplements =
    Array.isArray(supplements) && supplements.length > 0;

  const netBalance = Math.round(dailyTotalConsumed - dailySpentCalories);
  const balanceColor =
    netBalance < 0 ? "#2ecc71" : netBalance > 0 ? "#e74c3c" : themeColor;
  ...
  return (
    <SafeAreaView style={styles.container}>
      <Stack.Screen
        options={{ title: editMode ? "Modo de Edição" : "Histórico e Dados" }}
      />
        ...
        {isLoadingData ? (
          ...
          <>
            <View style={styles.calorieRow}>
              ...
              <Text style={[styles.calorieValue, { color: "#2ecc71" }]}>
                {dailyTotalConsumed} Kcal{" "}
              </Text>
            </View>
            <View style={styles.calorieRow}>
              <Text style={styles.calorieLabel}>Gastas:</Text>
              <Text style={[styles.calorieValue, { color: "#e74c3c" }]}>
                {dailySpentCalories} Kcal{" "}
              </Text>
            </View>
            <View style={styles.netBalanceRow}>
              <Text style={styles.netBalanceLabel}>Balanço Líquido: </Text>
              <Text style={[styles.netBalanceValue, { color: balanceColor }]}>
                {netBalance > 0 ? "+" : ""}
                {netBalance} Kcal
              </Text>
            </View>
      ...
      />
      {editMode && (
        <Text style={styles.editModeBanner}>
          Modo de Edição Ativado: Toque num registo para apagar.{" "}
        </Text>
      )}
          ...
          <Pressable style={styles.modalContent}>
            ...
            <View style={styles.modalBalanceCard}>
              <View style={styles.modalInfoBox}>
                ...
                <Text style={[styles.modalInfoValue, { color: "#2ecc71" }]}>
                  {modalConsumed} Kcal{" "}
                </Text>
              </View>
              <View style={styles.modalInfoBox}>
                <Text style={styles.modalInfoLabel}>Gasto </Text>
                <Text style={[styles.modalInfoValue, { color: "#e74c3c" }]}>
                  {modalSpent} Kcal{" "}
                </Text>
              </View>
              <View style={styles.modalInfoBox}>
                ...
                <Text
                  style={[styles.modalInfoValue, { color: modalBalanceColor }]}
                >
                  {modalNetBalance > 0 ? "+" : ""}
                  {modalNetBalance} Kcal{" "}
                </Text>
              </View>
            </View>
            ...
            {hasConfiguredSupplements && (
              <View style={styles.section}>
                ...
                {supplements.map((supplement) => {
                  const value =
                    supplementsOnSelectedDay[supplement.id] ??
                    (supplement.trackingType === "daily_check" ? false : 0);
                  ...
                  return (
                    <View key={supplement.id} style={styles.supplementItem}>
                      ...
                      {supplement.trackingType === "daily_check" ? (
                        ...
                        <Text
                          style={[
                            styles.supplementStatusValue,
                            { color: count > 0 ? themeColor : "gray" },
                          ]}
                        >
                          {count} dose(s){" "}
                        </Text>
            ...
            <View style={styles.section}>
              ...
              <FlatList
                ...
                keyExtractor={(item, index) => buildActivityKey(item, index)}
                ListEmptyComponent={
                ...
                renderItem={({ item }) => {
                  const workoutName = item.details?.type
                    ? workouts[item.details.type]?.name || item.details.type
                    : "";
                  const activityName =
                    item.category === "Musculação"
                      ? `Musculação (${workoutName})`
                      : item.category;
                  ...
                  if (isSwimmingWithDistance) {
                    detailParts.push(`${item.details.distance} m`);
                  }
                  if (isSwimmingWithDistance && item.details?.pace) {
                    detailParts.push(String(item.details.pace));
                  }
                  if (isRunningOrCycling && item.details?.distanceKm > 0) {
                    detailParts.push(`${item.details.distanceKm} km`);
                  }
                  if (duration) {
                    detailParts.push(`${duration} min`);
                  }
                  if (isRunningOrCycling && item.details?.pace) {
                    detailParts.push(String(item.details.pace));
                  }

                  const activityDetailString = detailParts.join(" / ");

                  return (
                    <Pressable
                      style={styles.activityItem}
                      onPress={
                        editMode ? () => handleDeleteActivity(item) : undefined
                      }
                    >
            ...
            {editMode && (
              ...
              >
                ...
                <Text style={styles.addButtonText}>
                  Adicionar Atividade a {selectedDay}{" "}
                </Text>
              </Pressable>
            )}
        ...
        >
          <Pressable style={styles.modalContent}>
            <Text style={styles.modalTitle}>
              Adicionar Atividade em {selectedDay}{" "}
            </Text>
            <FlatList
              ...
              renderItem={({ item }) => {
                ...
                return (
                  <Pressable
                    ...
                    onPress={() => handleAddActivitySelect(item)}
                  >
